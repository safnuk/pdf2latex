...
class Network:
    ...
    def build(self):
        ...
        self.training_sample = self.inference_sample_layer(decode_train)
        self.inference_sample = self.inference_sample_layer(decode_infer)
        ...
        for n in range(self.model.max_steps):
            if n % 10 == 0:
                ...
                if n % 100 == 99:  # Record execution stats
                    ...
                    summary, acc, loss, inferences = sess.run(
                        [merged, self.accuracy, self.training_loss,
    ...
    def inference_sample_layer(self, logits, skip=20):
        with tf.name_scope('sample'):
            indices = list(range(0, self.model.batch_size, skip))
            inferred = tf.gather(logits, indices)
            inferred = tf.argmax(inferred, 2)
            inferred = tf.cast(inferred, tf.int32)
            targets = tf.gather(self.target_sequences, indices)
            targets = tf.slice(targets, [0, 0], [-1, tf.shape(logits)[1]])
            return tf.stack([inferred, targets], axis=1)
    ...
    def create_rnn_decoder_cell(self):
        num_tokens = self.data.token_vocab_size
        with tf.variable_scope('decoder_cell'):
            cell = self._rnn_cell()
            self.decoder_cell = tf.contrib.rnn.OutputProjectionWrapper(
                cell, num_tokens)
...
class Model:
    ...
    @classmethod
    def small(cls):
        return cls('small', 100, 0.01, 1, 3000, 64, 1)
